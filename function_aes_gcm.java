...
public class function_aes_gcm {
    ...
    */
    ...
    try {
        if (ivAndCiphertext == null || ivAndCiphertext.length < IV_LENGTH + 1) {
            throw new IllegalArgumentException("Data terlalu pendek.");
        }

        byte[] iv = new byte[IV_LENGTH];
        System.arraycopy(ivAndCiphertext, 0, iv, 0, IV_LENGTH);

        byte[] ciphertextWithTag = new byte[ivAndCiphertext.length - IV_LENGTH];
        System.arraycopy(ivAndCiphertext, IV_LENGTH, ciphertextWithTag, 0, ciphertextWithTag.length);
        ...
        return cipher.doFinal(ciphertextWithTag);
            ...
            throw new RuntimeException("Tag GCM tidak valid (data berubah / key salah / IV salah).", e);
        } catch (Exception e) {
            throw new RuntimeException("Decrypt AES-GCM gagal", e);
        }
    }
